function varargout=integratebasis_up(CC,TH,J,phi,theta,Radius)
...
% Radius  The radius of Gaussian Smooth (unit: km) [DEFAULT: no smooth]
%
% OUTPUT:
%
...
defval('phi',0)
defval('theta',0)
defval('Radius',0) % default 500-km smooth
...
% consider the additional Gaussion Smooth
if length(J)==1
    if Radius>0
        % Run parallel is we are able (with Gaussian smooth)
        parfor h=1:J
            CC_smo=plm_Gausssmooth(CC{h},Radius)
            [Int,A]=plm2avg(CC_smo,XY);
            eigfunINT(h) = Int;
        end
    else
...
elseif length(J)==2
    ...
    % Run parallel is we are able (large orders of functions with Gaussian smooth)
    parfor h=J(1)+1:J(2)
        CC_smo=plm_Gausssmooth(CC{h},Radius)
        [Int,A]=plm2avg(CC_smo,XY);
        eigfunINT(h) = Int;
