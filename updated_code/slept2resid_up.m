function varargout=slept2resid_up(slept,thedates,fitwhat,givenerrors,specialterms,CC,TH,N,Radius)
...
% Radius      The radius of Gaussian Smooth (if applicable) [DEFAULT: no smooth]
%
% OUTPUT:
%
...
defval('extravalues',[]);
defval('Radius',0); % default no smooth
...
%        XY=eval(sprintf('%s(%i,%f)',TH{1},10,TH{2}));
        ...
        % for accuracy, we recommend not using spline interpolation
    ...
    % In case additional Gaussion smooth was needed
    % if only one N is provided, this is the same as the original procedure
    % if two N is provided, Gaussion smooth of a given radius was applied
    % to selective SSFs ( N(1) < N < N(2) )
    if length(N)==1
        N_all=N;
    elseif length(N)==2
        if N(1)>=N(2)
            error('Noted that N(2) should larger than N(1).')
        end
        N_all=N(2);
    else
        error('Wrong input of N.')
    end

%     [eigfunINT] = integratebasis(CC,TH,N);

    % consider cases with additional Gaussion smooth 
    [eigfunINT] = integratebasis_up(CC,TH,N,[],[],Radius);
    ...
    alphavarall = eigfunINT*Cab(1:N_all,1:N_all)*eigfunINT';
    ...
    totalparamerrors = paramerrors*365.25;
    % the length of year of 365.25 is probably more accurate than 365 considering Leap year
