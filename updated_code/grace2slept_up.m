function varargout=grace2slept_up(Dataproduct,TH,XY_buffer,Lwindow,phi,theta,omega,J,units,forcenew)
...
%                 [default: {'CSR', 'RL06', 60, 'land', 'Pelt17'} 
%                 which is Release level 06 data from the data center]
%                 at the Center for Space Research
%                 Another Example: {'CSR', 'RL06', 60, 'ocean', 'Pelt17'} 
%                 which is for ocean application
...
defval('Dataproduct',{'CSR', 'RL06', 60, 'land','Pelt17'})
defval('TH','greenland')
defval('Lwindow',18)
defval('phi',0)
defval('theta',0)
...
Ldata = Dataproduct{3};
land_or_ocean = Dataproduct{4}; % Here added coefficient land_or_ocean
GIA = Dataproduct{5}; % Here added GIA model
dataproductstring = [Pcenter Rlevel num2str(Ldata) land_or_ocean GIA];
% Here added coefficient land_or_ocean and GIA model
...
%     end
    ...
    % updated according to (https://github.com/csdms-contrib/slepian_delta/pull/17/commits/02a5935894302b13eedf728e26d948f803f501f9)
    % which is right if you use rather small buffering regions
    % Check if you want the Shannon number of eigenfunctions
    if strcmp(J,'N')
        J = round((Lwindow+1)^2*spharea(TH));
    end
...
%     [potcoffs,cal_errors,thedates]=grace2plmt(Pcenter,Rlevel,Ldata,units,0);
    ...
    % Added 'land_or_ocean' and GIA coefficient
    [potcoffs,cal_errors,thedates]=grace2plmt_up(Pcenter,Rlevel,Ldata,units,0,land_or_ocean,GIA);
